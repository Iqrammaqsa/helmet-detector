...
cap = cv2.VideoCapture("rtsp://service:Passw0rd!23@10.146.81.55:554/cam/realmonitor?channel=1&subtype=0")
# cap = cv2.VideoCapture(0)

no_helmet_timers = defaultdict(lambda: {"start": None, "captured": False})
person_data = {}       # person_id: {"bbox": (x1,y1,x2,y2), "last_seen": timestamp}
next_person_id = 0
iou_threshold = 0.5
inactive_timeout = 3   # Hapus ID jika tidak terlihat > 3 detik
...
while True:
    ...
    # Hapus ID yang sudah tidak terlihat
    ...
    for pid in new_person_ids:
        ...
        if wears_helmet:
            ...
            if no_helmet_timers[pid]["start"] is None:
                no_helmet_timers[pid]["start"] = current_time
            elif not no_helmet_timers[pid]["captured"] and current_time - no_helmet_timers[pid]["start"] > 5:
                ...
                no_helmet_timers[pid]["captured"] = True
